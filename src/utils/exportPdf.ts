...
/** Branded scorecard PDF — matches the original GRCArc export. */
export function exportScorecardPdf(state: ChecklistState, milestone: Milestone | 'none') {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.text(`GRCArc · IT Governance Scorecard · Page ${i} of ${pages}`, pageW / 2, doc.internal.pageSize.getHeight() - 24, { align: 'center' });
  }
  doc.save('grcarc-governance-scorecard.pdf');
}
...
export function exportGovernancePdf(data: {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(138, 157, 199);
    doc.text(`GRCArc Governance Report — Page ${i} of ${pages}`, pageW / 2, doc.internal.pageSize.getHeight() - 24, { align: 'center' });
  }
