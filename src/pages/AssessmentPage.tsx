...
export function AssessmentPage() {
  ...
  const [state, setState] = useState<ChecklistState>(() => {
    try { return JSON.parse(localStorage.getItem('grcarc_scorecard_v1') ?? '{}'); } catch { return {}; }
  });
  ...
  useEffect(() => {
    localStorage.setItem('grcarc_scorecard_v1', JSON.stringify(state));
  }, [state]);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="The original GRCArc IT Governance Checklist — work through it and export a branded PDF."
        action={
          <>
